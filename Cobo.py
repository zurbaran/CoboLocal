    ...
    while i < len(datoshistoricos):
        ...
        if not (MME is False):  # and len( datoshistoricos ) >= MME:
            ...
            if puntoMME > 0:
                ...
                elif maximo < puntoMME and (MME2 is False or (MME2 is not False and puntoMME <= puntoMME2)):
                ...
                while localizaLTi:
                    ...
                    for j in range(LTi, -1, -1):
                        ...
                        try:
                            ...
                            puntoLT = (minimoLTi * ((1 + (((1.0 + (((minimoLTf - minimoLTi) / minimoLTi))) ** (12.0 / (LTf - LTi))) - 1.0)) ** ((j - LTi) / 12.0)))
...
#                     while j<=i:
                    for j in range(LTf, i + 1):
                        ...
                        try:
                            ...
                            # Comprobamos si puntoLT es un número complejo
                            if isinstance(puntoLT, complex):
                                logging.debug(f"Se detectó un número complejo en el cálculo de puntoLT. Buscando LTf; Accion: {naccion}. Timming: {timming}. FechaLTi: {fechaLTi}. Fecha barra del Error: {fechaj}. Valores: minimoLTi={minimoLTi}, minimoLTf={minimoLTf}, LTf={LTf}, LTi={LTi}, j={j}")
            ...
            if s > 0:  # No podremos calcular LT si no hay barras fuerra del ciclo soporte y resistencia, por eso el soporte e inicio del ciclo tiene que ser mayor que 0
                ...
                while localizaLTi:
                    ...
                    for j in range(LTi, -1, -1):
                        ...
                        try:
                            # Realizamos el cálculo de puntoLT
                            puntoLT = (maximoLTi * ((1 + (((1.0 + (((maximoLTf - maximoLTi) / maximoLTi))) ** (12.0 / (LTf - LTi))) - 1.0)) ** ((j - LTi) / 12.0)))
