...
difregactualizar = {'d':10, 'w':15, 'm':30, 'noActualizados':120}# Expresa la diferencia entre los registros para hacer una actualizacion
pausareconexion = 20
...
#    desdeultimaactualizacionarchivo=(date(fechahoy[0],fechahoy[1],fechahoy[2])-date(fechaarchivo[0],fechaarchivo[1],fechaarchivo[2])).days
    ...
    if (desdeultimaactualizacion > difregactualizar[timming]):# and (desdeultimaactualizacionarchivo>difregistros):
        ...
        return ((datosaccion[-3][0]), timming, True)
    else:
...
def analisisAlcistaAccion(naccion, **config):
    ...
    if not(MME == False):
        puntosMME = indicador(datoshistoricos, MME = MME)
    ...
    while i < len(datoshistoricos):
        ...
        if not (MME == False):# and len( datoshistoricos ) >= MME:
            ...
            #if i >= ( MME - 1 ):# Empieza a utilizar el indicador en una barra en concreto, para la MME30 lo utiliza apartir de la barra 30(cuyo indice es 29)
            if puntoMME > 0:
    ...
    #                while j<=i:
            ...
            stoploss = round((cierreanterior - (puntoTAR * filtro)), 3)
            #stoploss = round((datoshistoricos[r][2] - (puntoTAR * filtro)), 3)
            listastoploss.append((fecha, stoploss))
            datoshistoricosTAR = fecha, stoploss, stoploss, stoploss, stoploss, volumensoporte
            #if volumen >= volumenanterior * 1.35:
...
def analisisBajistaAccion(naccion, **config):
    ...
    while i < len(datoshistoricos):
        ...
        if len(analisisbajista) > 1 and entradapuntoLT:
...
#            if LT: # esto es porque hasta que no se produce un segundo soporte, no podemos calcular la linea de tendencia
    ...
    #                        elif timming=='semanal':
            ...
            stoploss = round((datoshistoricos[s][3] + (puntoTAR * filtro)), 3)
...
def indicador(datos, **config):
    '''
    devuelve de la lista, datos, el indicador calculado
    indice, si es True devueve una tupla completa que corresponde al indicador con el formato (Fecha, indicador) para la lista de datos completa
    indice, si es un valor en concreto, nos devuelve el valor del indicador para ese indice en concreto
    '''
    ...
    if not indice == True:#devuelve el valor del indicador para ese indice en concreto
        resultado = puntoMME
...
def cruceconindicador(naccion):
    #TODO: funcion que compare el indicador con el precio de cotizacion actual de la accion para comprobar si esta por arriba, centrado o abajo de la ultima barra y con esta informacion, realizar el analisis alcista o bajista
    '''
    configurar el resultado que buscamos:
        Solo buscamos en el ultimo periodo mas actual almacenado, compare el indicador con el precio de cotizacion actual y la ultima barra del historico de la accion para comprobar si esta por arriba, centrado o abajo y distacia porcentual al precio y con esta informacion, realizar el analisis alcista o bajista y busquedas de valores de su situacion y cercania en concreto
...
def borraTicket (ticket, **config):
    ...
    if ticket != '' or ticket != ' ' or ticket != None:
        if BBDD:
            ...
            numeroResultado = cursor.execute(sql)
...
def cotizacionesMoneda(nombreticket):
    ...
    if '"No such ticker symbol.' in datosurl or 'Missing Symbols List.' in datosurl:#".DJA",".DJA",N/A,0,"N/A",N/A,N/A,N/A,N/A,0.00,"No such ticker symbol. <a href="/l">Try Symbol Lookup</a> (Look up: <a href="/l?s=.DJA">.DJA</a>)"
        ...
        #if numeroResultado == 0:#codigo     url_Inet     descripcion     valor     fechaRegistro
...
#        'E) Generar Archivos Grafico'
        if opcion == 'e':
            ...
            for n in datos:
                ...
                writercsv.writerow((naccion, fecha, '000000', apertura, maximo, minimo, cierre, volumen))
                #j.write(str(n)+'\n')
            j.close()

            MMEdatos = raw_input('Introduce Catidad de periodos para el indicador (30):')
            ...
            datosMME = indicador(datos, indice = True, MME = MMEdatos)
            ...
            TARdatos = raw_input('Introduce Catidad de periodos para el indicador (10):')
...
#        'O) Actualizar/Descargar Datos Cotizaciones Historicos todos los Tickets',
                ...
                if not ExistenDatos(ticket):
                    ...
                    for timmingdescargado in 'dwm':
                        ...
                        if actualizaractualizar:# and (desdefechamodificacionarchivo(datosaccion)):
                            ...
                            if fechaactualizar2 != None and actualizaractualizar == actualizaractualizar2 and fechaactualizar == fechaactualizar2 and timmingactualizar == timmingactualizar2:
                                fechahoy = ((date.today().timetuple()))
        ...
        if opcion == 'p':
            ...
            while len(listatickets) > 0:
                ...
                print('')
                print('Tickets pendientes de comprobar %d' % len(listatickets))
                print('Actualizando ticket ' , ticket)
        ...
        if opcion == 'q':
            ...
            for registro in listadetickets:
                ...
                if ExistenDatos(ticket):
                    ...
                    for timminganalisis in 'dwm':
                        #TODO: al final si utilizamos indicador, el indicador sera la decision de si es alcista o bajista
            ...
            for registro in resultado:
                ...
                if ExistenDatos(ticket):
                    ...
                    if estrategia == 'Alcista':
                        diario = analisisAlcistaAccion(ticket, timming = 'd', desdefecha = analizardesde, MME = MMe, conEntradaLT = EntradaLT, TAR = TAR, txt = True)
                        semanal = analisisAlcistaAccion(ticket, timming = 'w', desdefecha = analizardesde, MME = MMe, conEntradaLT = EntradaLT, TAR = TAR, txt = True)
                        mensual = analisisAlcistaAccion(ticket, timming = 'm', desdefecha = analizardesde, MME = MMe, conEntradaLT = EntradaLT, TAR = TAR, txt = True)
                    elif estrategia == 'Bajista':
                        diario = analisisBajistaAccion(ticket, timming = 'd', desdefecha = analizardesde, MME = MMe, conEntradaLT = EntradaLT, TAR = TAR, txt = True)
                        semanal = analisisBajistaAccion(ticket, timming = 'w', desdefecha = analizardesde, MME = MMe, conEntradaLT = EntradaLT, TAR = TAR, txt = True)
                        mensual = analisisBajistaAccion(ticket, timming = 'm', desdefecha = analizardesde, MME = MMe, conEntradaLT = EntradaLT, TAR = TAR, txt = True)
...
#                    '6) Diario con transicion a Semanal y Mensual'))
                    ...
                    #'1) Todo Semanal',
                    #'2) Todo Mensual',
                    #'3) Semanal con transicion a Mensual'
